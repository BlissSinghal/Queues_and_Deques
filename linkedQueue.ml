...
(* This problem extends the queue example that we covered in class
   ...
   We expect all solutions that use recursion to be implemented TAIL
   recursively, but this will generally not require any extra work:
   the intuitive, natural solutions are tail recursive. *)
...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  (* To check that a given queue is valid (i.e. that it satisfies the
     ...
     also keep track of which nodes have been traversed and must fail
     (and return None) if a cycle is detected.

     For acyclic queues, the function find_last_node below returns
     (Some n), where n is the tail of the queue found by traversing
     next pointers from qn until it finds the last node (i.e., a node
     whose 'next' pointer is None).  If it ever visits a node that has
     already been seen (indicating a cycle), it instead returns
     None. *)
  ...
  (* The next function, `valid`, checks whether a given queue
     ...
     Each of the functions defined below uses valid to check its
     arguments, as an aid to debugging.  In practice, doing such
     costly validity checking every time they are called would ruin
     the performance of the queue operations, so the checks would be
  ...
  (* Now it's your turn...
     ...
     a queue with v1 at the head and vn at the tail. Do not use any
  ...
  let contains (elt: 'a) (q: 'a queue) : bool =
    ...
    let rec loop (no: 'a qnode option) (elt: 'a) =
      begin match no with 
      |None -> false
      |Some n -> n.v == elt || loop n.next elt
      end 
    in loop q.head elt
  ...
  (* Implement a function that truncates a queue after the first
     ...
     element.) This means the specified element is still in the resulting
     queue. The function should mutate the queue "in place," --
     i.e., you should not construct a new queue, but instead should
     update the pointers within the given queue.

     Again, remember to use reference (==) equality! *)

  let truncate (elt: 'a) (q: 'a queue) : unit =
    if not (valid q) then failwith "truncate: given invalid queue";
    let rec loop (h: 'a qnode option) (elt: 'a) =
      begin match h with 
      |None -> ()
      |Some n -> 
        if n.v == elt then (n.next <- None; q.tail <- Some n)
        else loop n.next elt
      end
    in loop q.head elt
  ...
  let delete (elt: 'a) (q: 'a queue) : unit =
    let rec loop (curr: 'a qnode option)(prev: 'a qnode option) : unit =
      ...
      | Some n ->
        if n.v = elt then
          (*checking to see if we are at the tail*) 
          (begin match prev with 
          |None -> q.head <- n.next; 
            if n.next = None then q.tail <- None
            else ()
          |Some prevnode -> 
            if n.next = None then q.tail <- prev
            else ();
            prevnode.next <- n.next;
            (*what if the next node was also something to delete?*)
            loop prevnode.next (prev)
          end)
        else ();
        loop n.next curr
      end
  ...
  (* The provided "valid" function uses a helper function named
     ...
     or not a given queue has a cycle WITHOUT either using
     `contains_alias` or keeping a list (or other data structure)
     containing all the nodes that have already been seen. *)
