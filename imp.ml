...
let rec assoc (k: 'k) (l: ('k * 'v) list) : 'v option =
  begin match l with 
  |[] -> None
  |(key, v)::t -> (if key = k then Some v else assoc k t)
  end
...
let rec partial_transform (f: 'a -> 'b option) (l: 'a list) : 'b list =
  cat_option(transform (fun x -> f x) l)
...
(* Implement a function `iter` that iterates through each element of a list in
   order and calls a side-effecting function on each element. Do not use any
...
let swap (r1: state) (r2: state) : unit =
  let placeholder = r1.count in 
  r1.count <- r2.count; 
  r2.count <- placeholder;
  ()
...
(* Write a function that, given a value x and a list of values,
   ...
   Do NOT use the 'states_aliased' function to help with this
   problem. *)
...
(* Let's briefly review equality...  Two values are *structurally*
   ...
   ASM diagram!) *)
let equality_test_results () : (bool * bool) list =
  let r = { count = 5 } in
  let o = Some r in
  [
    (r=r, true) ;
...
let test () : bool =
  let combine (actual, expected) acc = (actual = expected) && acc in
  fold combine true (equality_test_results ())
...
let rec contains_alias_option (x: 'a) (l: 'a option list) : bool =
  begin match l with 
  |[] -> false
  |h::t -> let acc = contains_alias_option x t in
    begin match h with 
    |None -> acc
    |Some value -> x == value || acc
    end  
  end 
let test () : bool =
  let r = { count = 5 } in
...
let test () : bool =
  let r = { count = 5 } in
  let o = None in 
  let o1 = Some {count = 1} in
  let o2 = Some r in
  contains_alias_option r [o; o1; o2]
;; run_test "contains_option_alias multielement list contains alias" test
