...
module SimpleQueue : QueueInterface.QUEUE = struct
  ...
  (* Note: Feel free to use either higher order functions (e.g.,
     transform/fold) or List library functions. *)
  ...
  (* Truncate a queue after the first occurrence of a specified
     ...
     first occurrence of the given element). The specified element
     itself should still be in the resulting queue. Remember to use
     reference equality. (We want to use reference equality to ensure
     that the element "elt" refers to the same item on the heap as the
     element you are looking for, not just a copy.) *)
  let truncate (elt: 'a) (q: 'a queue) : unit =
    q.data <- fold (fun x acc -> if x == elt then [x] else x::acc) [] q.data
