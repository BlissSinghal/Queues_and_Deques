...
let test () : bool =
  ...
  begin match deq q with
  | None -> failwith "deq failed"
  | Some hd -> hd = 1 && to_list q  = [2]
  end
 ;; run_test "deq on multielement queue" test
...
let test() : bool = 
  ...
  truncate 3 q; 
  to_list q = []
;; run_test "truncate empty" test 

let test(): bool = 
  let q = from_list [5] in 
  truncate 5 q; 
  to_list q = [5]
;; run_test "truncate singleton, element exists" test

let test(): bool = 
  let q = from_list [5] in 
  truncate 3 q;
  to_list q = [5]
;; run_test "truncate singleton, element doesnt exist" test

let test(): bool = 
  let q = from_list [2; 4; 5; 4; 5] in 
  truncate 5 q; 
  to_list q = [2;4;5] 
;; run_test "truncate multielement list, with duplicates of truncating 
element" test
...
let test(): bool =
  ...
  delete 5 q;
  to_list q = []
;; run_test "delete singleton, element exists" test 

let test(): bool = 
  let q = from_list [5] in 
  delete 3 q;
  to_list q = [5] 
;; run_test "delete singleton, element doesn't exist" test 

let test(): bool = 
  let q = from_list [3;4] in 
  delete 4 q; 
  to_list q = [3]
...
let test(): bool = 
  let q = from_list [3;4;3] in 
  delete 3 q; 
  to_list q = [4]
;; run_test "delete head&tail" test

let test(): bool = 
  let q = from_list [3;4;5;5;3;5] in 
  delete 5 q; 
  to_list q = [3;4;3]
;; run_test "delete multielement list with duplicates" test
