...
(* A deque (pronounced 'deck') is like a queue, but with the ability
   ...
   using tail recursion, but this won't require any extra work if your
   solution is is the intuitive or natural solution to the question!
   Odds are, your solution is already tail recursive.
...
(* Check whether a given deque is valid. Checks invariants in both
   directions down the deque, then checks if the head and tail of the
   given deque match those returned by check_to_tail and
   check_to_head. *)
...
let deque_12 () : int deque =
  let node2 = {v = 2; prev = None; next = None} in 
  let node1 = {v = 1; prev = None; next = Some node2} in 
  let q = {head = Some node1; tail = Some node2} in
  (*need to update so that node2's prev points to node1*)
  begin match q.tail with 
  |None -> q
  |Some node -> node.prev <- Some node1; q
  end
...
let invalid_acyclic () : int deque =
  let node2 = {v = 2; prev = None; next = None} in 
  let node1 = {v = 1; prev = None; next = Some node2} in 
  (*node2 doesn't point back to node1*)
  {head = Some node1; tail = Some node2}
...
let cycle () : int deque =
  (*tail doesn't point to corect node?*)
  let node2 = {v = 2; prev = None; next = None} in 
  let node1 = {v = 1; prev = None; next = Some node2} in 
  let q = {head = Some node1; tail = None} in
  (*need to update so that node2's prev points to node1*)
  ...
  |Some node -> 
    (*go to the next node*)
     ...
     |None -> q
     |Some node2 -> node2.prev <- Some node; q
     end 
  end

let test() = 
  not (valid (cycle()))
;; run_test "invalid deque with cycle" test
...
(* Write a function to return a list of elements in the deque, ordered
   ...
   not use or reimplement the List library functions, @, or reverse
   (List.rev).

   HINT: Why are those List operations needed for singly-linked
   queues? *)
let to_list (q: 'a deque) : 'a list =
  if not (valid q) then failwith "to_list: given invalid deque";
  let rec loop (no: 'a dqnode option) (curr_list: 'a list): 'a list = 
    begin match no with 
    |Some n -> loop n.prev (n.v::curr_list)
    |None -> curr_list
    end
  in loop q.tail []
...
let insert_head (x: 'a) (q: 'a deque) : unit =
  ...
  (*need to make a new node for x*)
  let node = q.head in
  let newnode = {v = x; prev = None; next = node} in 
  q.head <- Some newnode; 
...
let insert_tail (x: 'a) (q: 'a deque) : unit =
  let node = q.tail in
  let newnode = {v = x; prev = node; next = None} in 
  q.tail <- Some newnode; 
  begin match node with 
  |None -> ()
  |Some n -> 
...
let remove_tail (q: 'a deque) : 'a =
  ...
  |Some n -> 
    let value: 'a = n.v in 
    (*update the tail to be the prev*)
    q.tail <- n.prev; 
    (*update the next to be nothing*)
    begin match q.tail with 
    |None -> q.head <- None
    |Some node2 -> 
      node2.next <- None
...
let delete (q: 'a deque) (startnode: 'a dqnode option) (go_next: bool) 
(elt: 'a) = 
  let rec loop (elt: 'a) (no: 'a dqnode option) = 
    begin match no with 
    |None -> ()
    |Some node -> 
      (*check if its equal to the element*)
      (*if so, we want to update the pointers of the prev and the next *)
      if node.v = elt then 
        begin match node.prev, node.next with 
        (*no nodes left in the q; q is now empty*)
        |None, None -> q.head <- None; q.tail <- None
        (*there were two nodes in the queue; the second node got deleted
        update the tail*)
        |Some n1, None -> n1.next <- None; q.tail <- Some n1
        (*there were two nodes in queue; first node got deleted; 
        update the head*)
        |None, Some n2 -> n2.prev <- None; q.head <- Some n2
        |Some n1, Some n2 -> n1.next <- Some n2; n2.prev <- Some n1
        end 
      else if go_next then loop elt node.next
      else loop elt node.prev
    end
  in loop elt startnode

let delete_last (v: 'a) (q: 'a deque) : unit =
  if not (valid q) then failwith "delete_last: given invalid deque";
  delete q q.tail false v
  (*
  let rec loop (elt: 'a) (no: 'a dqnode option) = 
    begin match no with 
    |None -> ()
...
(*for each element in queue, we are going to swap the .prev and .next pointers*)
  let rec loop (no: 'a dqnode option) =
    ...
    |Some n -> 
      ...
      |Some n1, Some n2 -> 
        let placeholder = n.prev in
        n.prev <- n.next; 
        n.next <- placeholder 
      (*the node was the tail, should now be the head*)
      |Some n1, None -> 
        n.next <- n.prev;
        n.prev <- None;
        (*updating q.head*)
        q.head <- no
      (*the node was the head, should now be the tail*)
      |None, Some n2 -> 
        n.prev <- n.next; 
        n.next <- None; 
        q.tail <- no
      ...
      (*calling the function on the prev now bc we switched the pointers*)
      loop n.prev
